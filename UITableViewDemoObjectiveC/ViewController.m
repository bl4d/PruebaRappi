...
-(UIView *) createItemView
{
  ...
  itemView.tag = kItemViewIndex++;
    ...
    UIImage *myimg = [UIImage imageNamed:@"75x75bb-85.png"];
    ...
    [itemView bringSubviewToFront:imageview];
...
- (void)viewDidLoad
{
    ...
    NSLog(@"Llamando al Servicio");
    [ServiceCaller executeRequest];
  ...
  frame = CGRectMake(viewFrame.size.width/2, 200, viewFrame.size.width/2, 200);
  frame = CGRectInset(frame, 20.0, 20.0);
  rightView = [[UIScrollView alloc] initWithFrame:frame];
  rightView.autoresizingMask = UIViewAutoresizingFlexibleWidth | UIViewAutoresizingFlexibleHeight | UIViewAutoresizingFlexibleBottomMargin;
